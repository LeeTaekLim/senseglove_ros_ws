...
void SenseGloveHardwareInterface::updateSenseGloveState()
{
  ...
  for (size_t i = 0; i < num_gloves_; ++i)
  {
    ...
    for (size_t k = 0; k < num_joints_; ++k)
    {
      ...
      senseglove_state_pub_->msg_.hand_angle[k].x = robot.getHandAngle(k).x;
      senseglove_state_pub_->msg_.hand_angle[k].y = robot.getHandAngle(k).y;
      senseglove_state_pub_->msg_.hand_angle[k].z = robot.getHandAngle(k).z;
